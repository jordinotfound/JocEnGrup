class GameScene extends Phaser.Scene {
    ...
    create (){	
		...
		var json = sessionStorage.getItem("config") || '{"cards":2,"dificulty":"easy", "puntsPerd":10}';
		var options_data = JSON.parse(json);
		var cuantosPares = options_data.cards;
		var dificultat = options_data.dificulty;
		var puntuacioPerd = options_data.puntsPerd;
				...
				if (this.firstClick){
					if (this.firstClick.card_id !== card.card_id){
						...
						if (this.correct >= cuantosPares){
							...
							options_data.cards = cuantosPares;
							options_data.dificulty = dificultat;
							options_data.puntsPerd = puntuacioPerd;	
							sessionStorage.setItem("config", JSON.stringify(options_data));
